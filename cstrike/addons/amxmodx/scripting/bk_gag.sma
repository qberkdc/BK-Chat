...
public cmd_gag(id) {
	...
	if(is_user_bot(target)) {
		client_print(id, print_console, "[BK-GAG] User is bot");
		return PLUGIN_HANDLED;
	}
	
	if(!amount) {
		client_print(id, print_console, "[BK-GAG] You are negative amount entered");
		return PLUGIN_HANDLED;
	}
	...
	if(datas[target] > timestamp) {
		client_print(id, print_console, "[BK-GAG] This player already gagged");
		return PLUGIN_HANDLED;
	}
...
public cmd_ungag(id) {
	...
	if(datas[target] < timestamp) {
		client_print(id, print_console, "[BK-GAG] This player already ungagged");
		return PLUGIN_HANDLED;
	}
