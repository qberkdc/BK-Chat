...
#define PLUGIN "[BK] Advanced Chat"
#define VERSION "1.20"
#define AUTHOR "Berk"
...
new const CFGNAME[] = "bkchat.cfg"
new const CUSTOM_CHAT_SOUND[] = "sound/cht/chat.wav"
...
public plugin_init()
{
	...
	register_concmd("bk_tag_add", "cmd_add_tag")
	register_concmd("bk_cfg_tag_add", "auto_cmd_add_tag")
	register_concmd("bk_tag_reload", "ReloadTags")
	...
	//	Cvars
	register_cvar("bk_flood_delay", "1.25")
	register_cvar("bk_spam_limit", "1")
	register_cvar("bk_dead_chat", "1")
	register_cvar("bk_custom_chat_sound", "1")
	register_cvar("bk_auto_advers_time", "0") 	// If you want do not send message, use zero value: 0 / default: 80
	register_cvar("bk_auto_advers_msg", "^^0[^^2%s^^0] ^^7This server using ^^3BK Chat 1.0^^7")
}
...
public plugin_cfg()
{
	...
	if(get_cvar_num("bk_auto_advers_time")) {
		set_task(float(get_cvar_num("bk_auto_advers_time")), "AutoAds", _, _, _, "b"); }
		
	if(get_cvar_num("bk_custom_chat_sound")) {
		StartPrecache(); }
}
...
public AutoAds()
{
	if(get_cvar_num("bk_auto_advers_time"))
	{
		new txt_m[562]; get_cvar_string("bk_auto_advers_msg", txt_m, charsmax(txt_m));
...
public cmd_add_tag(id)
{
	...
	new addcmd[128]
	formatex(addcmd, 127, "bk_cfg_add_tag ^"%s^" ^"%s^"", Arg[0], Arg[1])
...
public SayMsg(id)
{
	...
	formatex(Message, 253, "%s", Args)
	
	if(equali(Message, LastMsg[id]))
	{
		if(SpamBlock[id] < get_cvar_num("bk_spam_limit"))
			SpamBlock[id] += 1
	}
	else
	{
	...
	set_task(get_cvar_float("bk_flood_delay"), "un_flood", id)
	
	remove_color(Name, charsmax(Name));
	remove_color(Message, charsmax(Message));
...
stock Send(const team = 0, sender, type, const msg[], any:...)
{
	...
	if(get_gag(sender) > timestamp) {
		client_print(sender, print_chat, "[BK-GAG] You are gagged! timeleft: %s", fone);
		return PLUGIN_HANDLED;
	} else {
